...
export class HKTVMallHandler implements OrderChannelHandler {
  handle(model: OrderModel): OrderEntity {
    ...
    const shipmentStatus = EShipmentStatus.PENDING; // Assuming HKTVMall uses the same status for shipments
    const shipmentNumber = this.createShipmentNumber(); // Generate a unique shipment number
